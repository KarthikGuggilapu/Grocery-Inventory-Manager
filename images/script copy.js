...
function displayItems(data) {
  ...
  tableBody.innerHTML = data.map((item, index) => `
    <tr class="hover:bg-gray-50 transition-colors duration-150">
      <td class="p-3" data-label="#">${index + 1}</td>
      <td class="p-3" data-label="English Name">${escapeHtml(item.name_eng)}</td>
      <td class="p-3 telugu-text" data-label="Telugu Name">${escapeHtml(item.name_tel)}</td>
      <td class="p-3 price" data-label="Wholesale Price">₹${formatPrice(item.wholesale_price)}</td>
      <td class="p-3 price" data-label="Retail Price">₹${formatPrice(item.retail_price)}</td>
      <td class="p-3 flex justify-center gap-2" data-label="Actions">
        <button 
...
async function translateToTelugu(text) {
    try {
      const response = await fetch('https://api.anuvaad.org/transliterate', {
        method: 'POST',
      ...
      return data.transliteration ? data.transliteration : text;
    } catch (error) {
